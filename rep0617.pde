...
void blocks(){
  ...
  for (groundy=501; groundy<800; groundy=groundy+16*w) {
    for ( ground=0; ground<2300; ground=ground+16*w) {
      fill(42, 12, 0);
...
void draw(){
  ...
  //クリボー挙動
  if (kx<32*w||2300-32*w<kx) {
    kv=-kv;
  }
  kuribou(kx,ky);
  
  if(mx<0-16*w||2300+16*w<mx){
    mx=w*48;//マリオの初期化
    kx=2000;//クリボーの初期化
    my=500;
  }
  ...
  rightdokan();
  //土管に入る
  if(mx>2300-32*w&&my>500-30*w){
    mx=32*w;
    kx=2000;
    kv=2;
  }
  // //土管判定
 if(((mx<32*w)||(2300-32*w<mx))&&(my>=500-34*w)&&(my<=500-31*w)){
   g=0;
   Y0=500-31*w;
   v=0;
   t=0;
